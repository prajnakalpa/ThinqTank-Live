...
export async function POST(req: Request) {
  ...
  if (emailIdx === -1 || scoreIdx === -1) {
    return NextResponse.json({ error: 'CSV must have email and score columns' }, { status: 400 })
  }
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const email = rawEmail?.toLowerCase().trim() // ✅ ADDED (normalize)
    ...
    const { error } = await supabase
      ...
      .ilike('email', email) // ✅ ADDED (case-insensitive match)
