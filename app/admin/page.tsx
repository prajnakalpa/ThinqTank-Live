...
export default async function AdminDashboard() {
  ...
  return (
    <div>
      ...
      {/* Quick actions */}
      <div style={{ marginBottom: '2.5rem' }}>
        <h2 style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 600, fontSize: '1rem', color: '#94a3b8', marginBottom: 12, textTransform: 'uppercase', letterSpacing: '0.06em', fontSize: '0.75rem' }}>Quick Actions</h2>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          <Link href="/admin/activities" className="btn-primary" style={{ textDecoration: 'none', padding: '9px 20px', fontSize: '0.875rem' }}>+ New Activity</Link>
          <Link href="/admin/announcements" className="btn-ghost" style={{ textDecoration: 'none', padding: '9px 20px', fontSize: '0.875rem' }}>+ Announcement</Link>
          <Link href="/admin/settings" className="btn-ghost" style={{ textDecoration: 'none', padding: '9px 20px', fontSize: '0.875rem' }}>Edit Homepage</Link>
        </div>
      </div>
      ...
      <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(148,163,184,0.08)', borderRadius: 16, overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1.25rem 1.5rem', borderBottom: '1px solid rgba(148,163,184,0.06)' }}>
          <h2 style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 600, color: '#e2e8f0', fontSize: '0.95rem' }}>Recent Activities</h2>
          <Link href="/admin/activities" style={{ color: '#818cf8', fontSize: '0.8rem', textDecoration: 'none' }}>View all →</Link>
        </div>
        {!recentActivities?.length ? (
          <div style={{ padding: '3rem', textAlign: 'center', color: '#475569' }}>No activities yet.</div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid rgba(148,163,184,0.06)' }}>
                {['Title', 'Status', 'Participants', 'Avg Score', ''].map(h => (
                  <th key={h} style={{ padding: '10px 20px', textAlign: h === 'Participants' || h === 'Avg Score' ? 'right' : 'left', fontSize: '0.72rem', fontWeight: 700, color: '#334155', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {recentActivities.map((a: any) => {
                ...
                return (
                  <tr key={a.id} style={{ borderBottom: '1px solid rgba(148,163,184,0.04)', transition: 'background 0.15s' }}>
                    <td style={{ padding: '12px 20px', color: '#e2e8f0', fontSize: '0.9rem' }}>{a.title}</td>
                    <td style={{ padding: '12px 20px' }}>
                      <span style={{ display: 'inline-flex', alignItems: 'center', padding: '2px 10px', borderRadius: 99, fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', background: ss.bg, border: `1px solid ${ss.border}`, color: ss.color }}>{a.status}</span>
                    </td>
                    <td style={{ padding: '12px 20px', textAlign: 'right', color: '#64748b', fontSize: '0.85rem', fontFamily: 'monospace' }}>{stats?.participant_count ?? 0}</td>
                    <td style={{ padding: '12px 20px', textAlign: 'right', color: '#f59e0b', fontSize: '0.85rem', fontFamily: "'Space Grotesk', sans-serif", fontWeight: 600 }}>{stats?.avg_score ?? '—'}</td>
